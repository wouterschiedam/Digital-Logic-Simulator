use iced::{
    ...
    Color, Event, Rectangle, Renderer, Theme,
};
...
use crate::{
    helpers::{
        ...
        NODE_RADIUS, PROXIMITY_THRESHOLD,
    },
    linepath::LinePath,
    nodes::{Node, NodeType},
...
impl LogicGateApp {
    ...
    pub fn update_connections(connections: &mut [Connection], gates: &mut [LogicGate]) {
        ...
        for (gate_index, gate) in gates.iter().enumerate() {
            if let Some(output) = gate.inputs.get(0) {
                if output.state {
        ...
        for (gate_index, state) in updates {
            ...
            if let Some(input) = gates[gate_index].inputs.get_mut(0) {
                input.state = state;
            }
            // Update the output state of the gate receiving the input
            if let Some(output) = gates[gate_index].outputs.get_mut(0) {
                output.state = state;
            }
        }
    }
    ...
    fn find_node_at_position(
        ...
        // Iterate over each gate and its inputs/outputs
        for (gate_index, gate) in gates.iter().enumerate() {
                ...
                if self.is_dragging.get() {
                    *self.current_dragging_line.borrow_mut() = None;
                    ...
                    if let Some((gate_index, node_type)) = self.dragging_node.borrow().as_ref() {
                        ...
                        match node_type {
                            NodeType::Input => {
                                if let Some(node) = gate.inputs.iter_mut().find(|n| {
                                    is_point_near(
                            ...
                            NodeType::Output => {
                                if let Some(node) = gate.outputs.iter_mut().find(|n| {
                                    is_point_near(
                                        cursor.position_in(bounds).unwrap(),
                                        n.position.clone().into(),
                                        NODE_RADIUS,
